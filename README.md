...
Токен доступа **accessToken** отдается в теле запроса, в параметре **accessToken** и принимается в заголовке **authorization** в виде **bearer token**.

Токен обновления **refreshToken** записывается и передается в **cookie** в параметре **token**.

При каждом запросе происходит валидация обеих токенов, принятых от клиента. Валидация обязательно включает проверку ip-адреса и информацию о браузере клиента.

Если валидация прошла успешно, то запрос выполняется.
...
Далее производится поиск записи в таблице сессий по обеим токенам, а также по данным об id пользователя, ip-адресе и браузере клиента.
...
Новая пара токенов будет создаваться с каждым ответом. А на клиенте нужно будет реализовать прием и обновление **refreshToken** после каждого запроса.
...
В этом случае сервер не хранит токены и осуществляет только косвенную проверку. Безопасность хранения токенов ложится целиком на клиента.
