...
Сессия не содержит никаких персональных данных. Хранить другую информацию небезопасно, т.к. токен кодируется, но не шифруется. Но это и не нужно по причине того, что здесь реализуется только backend-часть.
...
- **JWT_SECRET** - задает секретное слово для подписи токена,
...
Токен кодируется, но не шифруется. Защита обеспечивается подписью, которая хранится только на сервере.
