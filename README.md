...
- [Встроенные возможности](#встроенные-возможности)
  - [Авторизация](#авторизация)
    ...
    - [Авторизация через соцсети](#авторизация-через-соцсети)
  - [Сессии](#сессии)
...
В таблице **auth** за такую авторизацию отвечают служебные поля:
...
При авторизации/регистрации создается новая сессия.
...
Часть этой информации дублируется в токене.
...
Сессия не содержит никаких персональных данных, кроме идентификатора пользователя. Это не нужно по причине того, что здесь реализуется только backend-часть.
...
Наличие сессии помогает убедиться, что запрос не был поддельным. И именно для этого обеспечивается ее защита.

[^ к оглавлению](#оглавление)

### Способы защиты

Защита современных систем строится на токенах или на сессиях.

В первом случае используются два **jwt** токена:

- access token - основной, для доступа к данным, с коротким временем жизни, порядка нескольких минут,
- refresh token - вспомогательный, с долгим временем жизни, до месяца, разрешает генерировать основной токен.

Такая система на наш взгляд имеет ряд недостатков.

Если access токен не проходит валидацию, клиенту возвращается сообщение об ошибке. Тогда от клиента должен поступить запрос на генерацию нового access токена. И только после этого запрос может быть выполнен повторно.

Учитывая короткое время жизни access токена, подобные события будут происходить регулярно. В результате клиент и сервер будут нагружены дополнительными проверками и запросами.

Также access token используется для безопасного хранения и передачи данных в сессии, чтобы сократить число запросов в базу данных.

Во-первых, объем таких данных неизбежно станет слишком большим. Во-вторых, данные могут перестать быть актуальными. А в-третьих, такое применение выглядит бессмысленным для API.

Другой недостаток заключается в том, что токен будет валидным все время жизни. Это значит, что пользователя нельзя будет разлогинить или запретить совершать определенные действия.

Избавление от этих недостатков приводит к реализации защиты на основе сессий.

Однако предлагаемый в express механизм сессий нас не вполне устраивает.

Мы используем собственную защиту, которая сочетает в себе оба способа.

[^ к оглавлению](#оглавление)

### Защита сессии

Защита сессии построена на основе токена **jwt** и секретного ключа **secret**.

Они оба сохраняются на сервере в базе данных.

Токен записывается и передается в **cookie** в параметре **token**. Секретный ключ принимается в теле запроса, в параметре **secret** и отдается в теле ответа.

При каждом запросе происходит валидация токена и секретного ключа, принятых от клиента. К тому же токен декодируется и его содержимое тоже проверяется.

Если валидация прошла успешно, то запрос выполняется. При этом генерируется новая пара токена и секретного ключа, которые передаются обратно клиенту.

Если валидация не пройдена, запрос отклоняется и сессия закрывается.

Закрытие сессии означает удаление всех данных текущей сессии из базы, после чего токены и секретные ключи перестают быть валидными.

Закрытием сессии можно управлять.

При таком подходе вы можете быть уверены в том, что корректная информация была отдана с первого раза. А значит, не требуется повторных запросов и дополнительных проверок. Новая пара токен + секретный ключ будет передана вам вместе с ответом, и на это тоже не требуется тратить лишние ресурсы.

Если же что-то пошло не так, то, возможно, ваша сессия подверглась взлому и ее нужно срочно закрыть.
...
Токен сочетает в себе механизмы refresh и access токенов.

Для генерации токена используются данные из переменных окружения:

- **JWT_SECRET** - задает секретное слово для кодирования/декодирования информации в токене,
- **JWT_EXPIRES** - задает время жизни токена.
...
Мы не рекомендуем делать время жизни токена слишком маленьким. Иначе пользователю придется часто выполнять авторизацию.

Токен содержит в себе следующую информацию об авторизации:

- userId - номер пользователя,
- ip - ip-адрес клиента,
- agent - информация о браузере клиента.

Токен кодируется и далее передается в зашифрованном виде. Расшифровать его можно только на сервере, потому что доступ к секретному слову есть только у сервера.

Токен сохраняется в **cookie** на клиенте и передается вместе с запросами. Поэтому вам не нужно предпринимать никаких дополнительных действий.

[^ к оглавлению](#оглавление)

### Секретный ключ

Секретный ключ используется вместо access токена. Это случайно сгенерированная строка, которая сама по себе не содержит никакой информации.

По-сути это одноразовый токен, который хранится в базе данных и меняется после каждого запроса.

Его преимущества:
- не занимает много места,
- является одноразовым,
- бесконечное время простоя,
- другой способ хранения,
- без него запрос не будет валидирован.

Вместе с генерацией секретного ключа заново генерируется и токен.

Мы оставили возможность генерации по **get** запросу, чтобы у вас была возможность использовать это на клиенте. Например, если вы не захотите отдавать ключ в ответе на каждый запрос.

Если генерацию по запросу нужно выключить, достаточно просто удалить или закомментировать соответствующий путь в файле **routes.js**.

При желании, вы можете передавать секретный ключ в формате **jwt**. Это имеет смысл, если вы хотите ограничить время жизни секретного ключа. Но в таком случае генерацию по **get** запросу нужно оставить.

[^ к оглавлению](#оглавление)

### Валидация

Вы можете изменить условия проверки токена и секретного ключа по своему желанию.

Например, вы можете отключить удаление сессии при неверном секретном ключе, чтобы пользователя не выбрасывало из приложения, а просто выдавало ошибку.
...
Методы задаются в файле **error.api.js**, middleware - в файле **error.middleware.js**.
